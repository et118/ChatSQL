...
def rebuild_if_not_initialized():
    ...
    CREATE TABLE IF NOT EXISTS Sessions (
        ...
        auth_token_hash VARCHAR(60) NOT NULL,
...
def is_auth_token_valid(auth_token, username):
    ...
    """, (HashManager.hash_string(auth_token), username))
...
def login(username, password):
    ...
    WHERE username = %s
    AND password_hash = %s
    """, (username, HashManager.hash_string(password)))
    ...
    if cursor.fetchone() is not None:
        ...
        """, (username, HashManager.hash_string(auth_token)))
...
def signup(username, email, password):
    ...
    if cursor.fetchone() is None: #If account with signup username does not exist, its valid, since we need unique usernames
        ...
        """, (username, email, HashManager.hash_string(password)))
        connection.commit()
